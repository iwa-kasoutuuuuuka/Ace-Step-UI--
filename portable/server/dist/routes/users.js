...
const router = Router();
async function resolvePublicAudioUrl(audioUrl) {
    if (!audioUrl)
        return null;
...
router.get('/public/featured', async (_req, res) => {
    try {
        ...
        let result = await pool.query(`SELECT u.id, u.username, u.bio, u.avatar_url, u.created_at,
        ...
        if (result.rows.length === 0) {
            result = await pool.query(`SELECT u.id, u.username, u.bio, u.avatar_url, u.created_at,
                        (SELECT COUNT(*) FROM followers WHERE following_id = u.id) as follower_count,
                        0 as song_count
                 FROM users u
...
router.get('/:username', optionalAuthMiddleware, async (req, res) => {
    try {
        const result = await pool.query(`SELECT u.id, u.username, u.created_at, u.bio, u.avatar_url, u.banner_url
             FROM users u
             WHERE u.username = $1`, [req.params.username]);
        if (result.rows.length === 0) {
...
router.get('/:username/songs', async (req, res) => {
    try {
        ...
        const songsResult = await pool.query(`SELECT s.id, s.title, s.lyrics, s.style, s.caption, s.cover_url, s.audio_url,
...
router.get('/:username/playlists', async (req, res) => {
    try {
        ...
        const userId = userResult.rows[0].id;
        const playlistsResult = await pool.query(`SELECT p.id, p.name, p.description, p.cover_url, p.created_at,
...
router.post('/me/avatar', authMiddleware, upload.single('avatar'), async (req, res) => {
    try {
        ...
        // Update user record
        const result = await pool.query(`UPDATE users SET avatar_url = $1, updated_at = datetime('now') WHERE id = $2 RETURNING id, username, created_at, bio, avatar_url, banner_url`, [url, userId]);
...
router.post('/me/banner', authMiddleware, upload.single('banner'), async (req, res) => {
    try {
        ...
        const url = storage.getPublicUrl(key);
        // Update user record
        const result = await pool.query(`UPDATE users SET banner_url = $1, updated_at = datetime('now') WHERE id = $2 RETURNING id, username, created_at, bio, avatar_url, banner_url`, [url, userId]);
        res.json({ user: result.rows[0], url });
    }
    catch (error) {
...
router.patch('/me', authMiddleware, async (req, res) => {
    try {
        ...
        const result = await pool.query(`UPDATE users SET ${updates.join(', ')} WHERE id = $${paramCount} RETURNING id, username, created_at, bio, avatar_url, banner_url`, values);
...
router.post('/:username/follow', authMiddleware, async (req, res) => {
    try {
        ...
        const existingFollow = await pool.query('SELECT 1 FROM followers WHERE follower_id = $1 AND following_id = $2', [currentUserId, targetUserId]);
        let following = false;
        if (existingFollow.rows.length > 0) {
            // Unfollow
            await pool.query('DELETE FROM followers WHERE follower_id = $1 AND following_id = $2', [currentUserId, targetUserId]);
            following = false;
        }
        else {
            // Follow
            await pool.query('INSERT INTO followers (follower_id, following_id) VALUES ($1, $2)', [currentUserId, targetUserId]);
            following = true;
        }
        // Get updated follower count
        const countResult = await pool.query('SELECT COUNT(*) as count FROM followers WHERE following_id = $1', [targetUserId]);
        res.json({
            following,
...
router.get('/:username/followers', async (req, res) => {
    try {
        ...
        const userId = userResult.rows[0].id;
        const followersResult = await pool.query(`SELECT u.id, u.username, u.created_at
             FROM followers f
...
router.get('/:username/following', async (req, res) => {
    try {
        ...
        const userId = userResult.rows[0].id;
        const followingResult = await pool.query(`SELECT u.id, u.username, u.created_at
             FROM followers f
...
router.get('/:username/stats', optionalAuthMiddleware, async (req, res) => {
    try {
        const userResult = await pool.query('SELECT id FROM users WHERE username = $1', [req.params.username]);
        if (userResult.rows.length === 0) {
            res.status(404).json({ error: 'User not found' });
        ...
        const [followerCountResult, followingCountResult, isFollowingResult] = await Promise.all([
            ...
            req.user
                ? pool.query('SELECT 1 FROM followers WHERE follower_id = $1 AND following_id = $2', [req.user.id, userId])
