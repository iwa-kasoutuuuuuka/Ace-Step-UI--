...
const audioStorage = multer.diskStorage({
    destination: async (_req, _file, cb) => {
        const datasetName = _req.body?.datasetName || 'default';
...
function getAudioDuration(filePath) {
    try {
        const result = execSync(`ffprobe -v error -show_entries format=duration -of default=noprint_wrappers=1:nokey=1 "${filePath}"`, { encoding: 'utf-8', timeout: 10000 });
...
router.post('/upload-audio', authMiddleware, audioUpload.array('audio', 50), async (req, res) => {
    try {
        ...
        const datasetName = req.body?.datasetName || 'default';
...
router.post('/build-dataset', authMiddleware, async (req, res) => {
    try {
        const { datasetName = 'my_lora_dataset', customTag = '', tagPosition = 'prepend', allInstrumental = true, } = req.body;
...
router.get('/audio', authMiddleware, async (req, res) => {
    try {
        ...
        if (req.query.path) {
            ...
            filePath = path.join(config.datasets.dir, req.query.file);
        }
        else {
...
router.post('/scan-directory', authMiddleware, async (req, res) => {
    try {
        const { audioDir, datasetName = 'my_lora_dataset', customTag = '', tagPosition = 'prepend', allInstrumental = true, } = req.body;
...
router.post('/auto-label', authMiddleware, async (req, res) => {
    try {
        const { skipMetas = false, formatLyrics = false, transcribeLyrics = false, onlyUnlabeled = false, } = req.body;
...
router.post('/init-model', authMiddleware, async (req, res) => {
    try {
        const { checkpoint, configPath, device = 'auto', initLlm = false, lmModelPath = '', backend = 'pt', useFlashAttention = false, offloadToCpu = false, offloadDitToCpu = false, compileModel = false, quantization = false, } = req.body;
        const client = await getGradioClient();
        try {
...
router.get('/checkpoints', authMiddleware, async (_req, res) => {
    try {
        const aceStepDir = getAceStepDir();
        ...
        const configDirs = entries.filter(e => e.startsWith('acestep-v15') && statSync(path.join(checkpointDir, e)).isDirectory());
...
router.get('/lora-checkpoints', authMiddleware, async (req, res) => {
    try {
        const outputDir = req.query.dir || './lora_output';
        const aceStepDir = getAceStepDir();
...
router.post('/load-dataset', authMiddleware, async (req, res) => {
    try {
        ...
        res.json({
            status: data[0],
            dataframe: data[1],
            sampleCount: Array.isArray(data[1]?.data) ? data[1].data.length : 0,
            sample: {
                index: data[2],
                audio: data[3],
                filename: data[4],
...
router.get('/sample-preview', authMiddleware, async (req, res) => {
    try {
...
router.post('/save-sample', authMiddleware, async (req, res) => {
    try {
...
router.post('/update-settings', authMiddleware, (_req, res) => {
...
router.post('/save-dataset', authMiddleware, async (req, res) => {
    try {
        ...
        if (customTag !== undefined)
            body.custom_tag = customTag;
        if (tagPosition !== undefined)
            body.tag_position = tagPosition;
        if (allInstrumental !== undefined)
            body.all_instrumental = allInstrumental;
        if (genreRatio !== undefined)
            body.genre_ratio = genreRatio;
...
router.post('/load-tensors', authMiddleware, async (req, res) => {
    try {
...
router.post('/start', authMiddleware, async (req, res) => {
    try {
        const { tensorDir, rank, alpha, dropout, learningRate, epochs, batchSize, gradientAccumulation, saveEvery, shift, seed, outputDir, resumeCheckpoint, } = req.body;
        const client = await getGradioClient();
...
router.post('/stop', authMiddleware, async (_req, res) => {
    try {
        const client = await getGradioClient();
...
router.post('/import-dataset', authMiddleware, async (req, res) => {
    try {
