...
import { generateMusicViaAPI, getJobStatus, getAudioStream, discoverEndpoints, checkSpaceHealth, cleanupJob, getJobRawResponse, downloadAudioToBuffer, resolvePythonPath, } from '../services/acestep.js';
...
router.post('/upload-audio', authMiddleware, (req, res, next) => {
...
router.post('/', authMiddleware, async (req, res) => {
    try {
        const { customMode, songDescription, lyrics, style, title, instrumental, vocalLanguage, duration, bpm, keyScale, timeSignature, inferenceSteps, guidanceScale, batchSize, randomSeed, seed, thinking, audioFormat, inferMethod, shift, lmTemperature, lmCfgScale, lmTopK, lmTopP, lmNegativePrompt, lmBackend, lmModel, referenceAudioUrl, sourceAudioUrl, referenceAudioTitle, sourceAudioTitle, audioCodes, repaintingStart, repaintingEnd, instruction, audioCoverStrength, taskType, useAdg, cfgIntervalStart, cfgIntervalEnd, customTimesteps, useCotMetas, useCotCaption, useCotLanguage, autogen, constrainedDecodingDebug, allowLmBatch, getScores, getLrc, scoreScale, lmBatchChunkSize, trackName, completeTrackClasses, isFormatCaption, ditModel, } = req.body;
        ...
        await pool.query(`INSERT INTO generation_jobs (id, user_id, status, params, created_at, updated_at)
       VALUES (?, ?, 'queued', ?, datetime('now'), datetime('now'))`, [localJobId, req.user.id, JSON.stringify(params)]);
        // Start generation
        const { jobId: hfJobId } = await generateMusicViaAPI(params);
        // Update job with ACE-Step task ID
        await pool.query(`UPDATE generation_jobs SET acestep_task_id = ?, status = 'running', updated_at = datetime('now') WHERE id = ?`, [hfJobId, localJobId]);
        res.json({
            jobId: localJobId,
            status: 'queued',
            queuePosition: 1,
...
router.get('/status/:jobId', authMiddleware, async (req, res) => {
    try {
        const jobResult = await pool.query(`SELECT id, user_id, acestep_task_id, status, params, result, error, created_at
        ...
        if (['pending', 'queued', 'running'].includes(job.status) && job.acestep_task_id) {
            try {
                ...
                if (aceStatus.status !== job.status) {
                    ...
                    const updateParams = [aceStatus.status];
                    ...
                    if (aceStatus.status === 'succeeded' && aceStatus.result && wasUpdated) {
                        ...
                        const audioUrls = aceStatus.result.audioUrls.filter((url) => {
                        ...
                        for (let i = 0; i < audioUrls.length; i++) {
                            ...
                            try {
                                ...
                                await pool.query(`INSERT INTO songs (id, user_id, title, lyrics, style, caption, audio_url,
...
router.get('/history', authMiddleware, async (req, res) => {
    try {
        const result = await pool.query(`SELECT id, acestep_task_id, status, params, result, error, created_at
       FROM generation_jobs
       WHERE user_id = ?
...
router.get('/endpoints', authMiddleware, async (_req, res) => {
    try {
...
router.get('/models', async (_req, res) => {
    try {
        ...
        models.sort((a, b) => {
            ...
            if (a.is_preloaded !== b.is_preloaded)
                return a.is_preloaded ? -1 : 1;
...
router.get('/random-description', authMiddleware, async (_req, res) => {
    try {
...
router.get('/health', async (_req, res) => {
    try {
        ...
        res.json({ healthy: false, aceStepUrl: config.acestep.apiUrl, error: error.message });
    }
});
...
router.get('/debug/:taskId', authMiddleware, async (req, res) => {
    try {
...
router.post('/format', authMiddleware, async (req, res) => {
    try {
        ...
        if (duration && duration > 0)
            paramObj.duration = duration;
        if (keyScale)
            paramObj.key = keyScale;
        if (timeSignature)
            paramObj.time_signature = timeSignature;
        ...
        if (keyScale)
            args.push('--key-scale', keyScale);
        if (timeSignature)
            args.push('--time-signature', timeSignature);
        ...
        if (lmModel)
            args.push('--lm-model', lmModel);
        if (lmBackend)
            args.push('--lm-backend', lmBackend);
        ...
        const result = await new Promise((resolve) => {
            ...
            proc.on('close', (code) => {
                if (code === 0 && stdout) {
                    ...
                    if (stdout)
                        console.error('[Format] stdout:', stdout.slice(0, 1000));
                    if (stderr)
                        console.error('[Format] stderr:', stderr.slice(0, 1000));
