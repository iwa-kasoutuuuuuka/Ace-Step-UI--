...
async function resolveAccessibleAudioUrl(audioUrl, isPublic) {
    if (!audioUrl)
        return null;
    if (audioUrl.startsWith('s3://')) {
        const storageKey = audioUrl.replace('s3://', '');
...
router.get('/:id/audio', optionalAuthMiddleware, async (req, res) => {
    try {
        const result = await pool.query(`SELECT s.audio_url, s.is_public, s.user_id FROM songs s WHERE s.id = $1`, [req.params.id]);
...
router.get('/', authMiddleware, async (req, res) => {
    try {
        const result = await pool.query(`SELECT s.id, s.title, s.lyrics, s.style, s.caption, s.cover_url, s.audio_url,
...
router.get('/public/featured', optionalAuthMiddleware, async (_req, res) => {
    try {
...
router.get('/:id', optionalAuthMiddleware, async (req, res) => {
    try {
        const result = await pool.query(`SELECT s.id, s.user_id, s.title, s.lyrics, s.style, s.caption, s.cover_url, s.audio_url,
...
router.get('/:id/full', optionalAuthMiddleware, async (req, res) => {
    try {
        const [songResult, commentsResult] = await Promise.all([
            pool.query(`SELECT s.id, s.user_id, s.title, s.lyrics, s.style, s.caption, s.cover_url, s.audio_url,
                s.duration, s.bpm, s.key_scale, s.time_signature, s.tags, s.is_public,
         ...
         LEFT JOIN users u ON s.user_id = u.id
         WHERE s.id = $1`, [req.params.id]),
            pool.query(`SELECT c.id, c.content, c.created_at, c.updated_at,
...
router.post('/', authMiddleware, async (req, res) => {
    try {
        ...
        const result = await pool.query(`INSERT INTO songs (user_id, title, lyrics, style, caption, cover_url, audio_url,
...
// Get liked songs
router.get('/liked/list', authMiddleware, async (req, res) => {
    try {
        const result = await pool.query(`SELECT s.id, s.title, s.lyrics, s.style, s.cover_url, s.audio_url,
       ...
       LEFT JOIN users u ON s.user_id = u.id
       WHERE ls.user_id = $1
       ORDER BY ls.liked_at DESC`, [req.user.id]);
        const songs = await Promise.all(result.rows.map(async (row) => ({
            ...row,
            audio_url: await resolveAccessibleAudioUrl(row.audio_url, row.is_public),
        })));
...
// Track song play
router.post('/:id/play', optionalAuthMiddleware, async (req, res) => {
    try {
        const result = await pool.query(`UPDATE songs
       SET view_count = COALESCE(view_count, 0) + 1, updated_at = CURRENT_TIMESTAMP
       WHERE id = $1
       RETURNING view_count`, [req.params.id]);
        if (result.rows.length === 0) {
            res.status(404).json({ error: 'Song not found' });
            return;
        }
...
router.get('/:id/comments', optionalAuthMiddleware, async (req, res) => {
    try {
        const result = await pool.query(`SELECT c.id, c.content, c.created_at, u.username, u.id as user_id, u.avatar_url
       FROM comments c
       JOIN users u ON c.user_id = u.id
       WHERE c.song_id = $1
       ORDER BY c.created_at DESC`, [req.params.id]);
...
router.post('/:id/comments', authMiddleware, async (req, res) => {
    try {
        ...
        const result = await pool.query(`INSERT INTO comments (song_id, user_id, content)
       VALUES ($1, $2, $3)
       RETURNING id, content, created_at`, [req.params.id, req.user.id, content.trim()]);
        const comment = {
            ...result.rows[0],
