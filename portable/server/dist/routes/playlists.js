...
router.post('/', authMiddleware, async (req, res) => {
    try {
        ...
        const result = await pool.query(`INSERT INTO playlists (user_id, name, description, is_public, cover_url)
             VALUES ($1, $2, $3, $4, $5)
             RETURNING *`, [req.user.id, name, description, isPublic || false, coverUrl]);
...
router.get('/', authMiddleware, async (req, res) => {
    try {
        const result = await pool.query(`SELECT p.*, COUNT(ps.song_id) as song_count
             FROM playlists p
...
router.get('/public/featured', async (_req, res) => {
    try {
        ...
        let result = await pool.query(`SELECT p.id, p.name, p.description, p.cover_url, p.created_at,
        ...
        if (result.rows.length === 0) {
            result = await pool.query(`SELECT p.id, p.name, p.description, p.cover_url, p.created_at,
                        u.username as creator, u.avatar_url as creator_avatar,
                        0 as song_count
                 FROM playlists p
                 JOIN users u ON p.user_id = u.id
...
router.get('/:id', optionalAuthMiddleware, async (req, res) => {
    try {
        const playlistResult = await pool.query(`SELECT p.*, u.username as creator, u.avatar_url as creator_avatar
             FROM playlists p
             JOIN users u ON p.user_id = u.id
        ...
        // Get songs
        const songsResult = await pool.query(`SELECT s.id, s.title, s.lyrics, s.style, s.cover_url, s.audio_url, s.duration,
                    s.user_id, s.is_public, u.username as creator, ps.added_at, ps.position
             FROM playlist_songs ps
             JOIN songs s ON ps.song_id = s.id
             JOIN users u ON s.user_id = u.id
             WHERE ps.playlist_id = $1
             ORDER BY ps.position ASC`, [req.params.id]);
...
router.post('/:id/songs', authMiddleware, async (req, res) => {
    try {
        ...
        // Get max position
        const positionResult = await pool.query('SELECT MAX(position) as max_pos FROM playlist_songs WHERE playlist_id = $1', [req.params.id]);
        const position = (positionResult.rows[0].max_pos || 0) + 1;
        await pool.query(`INSERT INTO playlist_songs (playlist_id, song_id, position)
             VALUES ($1, $2, $3)
             ON CONFLICT (playlist_id, song_id) DO NOTHING`, [req.params.id, songId, position]);
