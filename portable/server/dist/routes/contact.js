...
router.post('/', async (req, res) => {
    try {
        ...
        const result = await pool.query(`INSERT INTO contact_submissions (name, email, subject, message, category)
       VALUES ($1, $2, $3, $4, $5)
       RETURNING id, created_at`, [name, email, subject, message, category || 'general']);
        res.status(201).json({
            success: true,
...
router.patch('/admin/:id/read', adminMiddleware, async (req, res) => {
    try {
        const { id } = req.params;
        const { isRead } = req.body;
        const result = await pool.query(`UPDATE contact_submissions SET is_read = $1 WHERE id = $2 RETURNING is_read`, [isRead, id]);
...
router.get('/admin/unread-count', adminMiddleware, async (_req, res) => {
    try {
        const result = await pool.query(`
