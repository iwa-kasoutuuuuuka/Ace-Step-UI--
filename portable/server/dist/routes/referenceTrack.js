...
const findWhisperExecutable = async () => {
...
const transcribeWithWhisper = async (buffer, originalFilename, signal) => {
...
router.get('/', authMiddleware, async (req, res) => {
    try {
        const result = await pool.query(`SELECT id, filename, storage_key, duration, file_size_bytes, tags, created_at
...
router.post('/', authMiddleware, upload.single('audio'), async (req, res) => {
    try {
        ...
        const result = await pool.query(`INSERT INTO reference_tracks (user_id, filename, storage_key, file_size_bytes, tags)
       VALUES ($1, $2, $3, $4, $5)
       RETURNING *`, [userId, originalFilename, key, req.file.size, tags]);
        res.status(201).json({
            track: {
                ...result.rows[0],
                audio_url: audioUrl
