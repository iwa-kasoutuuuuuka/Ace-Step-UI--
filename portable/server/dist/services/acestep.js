...
export function resolvePythonPath(baseDir) {
 ...
 */
async function prepareAudioFile(audioUrl) {
    if (!audioUrl)
        return null;
 ...
 */
async function buildGradioArgs(params) {
 ...
 */
async function downloadGradioAudioFile(fileObj, destPath) {
...
async function switchModelIfNeeded(ditModel) {
...
export async function generateMusicViaAPI(params) {
...
// ---------------------------------------------------------------------------
async function processGeneration(jobId, params, job) {
    job.status = 'running';
...
async function processGenerationViaGradio(jobId, params, job) {
    ...
    const genDetails = data[9];
    const genStatus = data[10];
    // Collect audio file objects — prefer the "All Generated Files" list
    let audioFileObjects = [];
    if (Array.isArray(allFiles) && allFiles.length > 0) {
        audioFileObjects = allFiles.filter((f) => f && (f.path || f.url) && isAudioFile(f.orig_name || f.path || ''));
    }
...
function parseGenerationDetails(details) {
    if (!details)
        return {};
    try {
...
// ---------------------------------------------------------------------------
async function processGenerationViaPython(jobId, params, job) {
    const caption = params.style || 'pop music';
    const prompt = params.customMode ? caption : (params.songDescription || caption);
    const lyrics = params.instrumental ? '' : (params.lyrics || '');
    ...
    try {
        ...
        if (resolvedTaskType && resolvedTaskType !== 'text2music')
            args.push('--task-type', resolvedTaskType);
...
function runPythonGeneration(scriptArgs, timeoutMs = 600000) {
...
export function getJobRawResponse(jobId) {
    const job = activeJobs.get(jobId);
...
// ---------------------------------------------------------------------------
export async function getAudioStream(audioPath) {
...
export async function downloadAudio(remoteUrl, songId) {
    await mkdir(AUDIO_DIR, { recursive: true });
...
export async function downloadAudioToBuffer(remoteUrl) {
    const response = await getAudioStream(remoteUrl);
    if (!response.ok) {
        throw new Error(`Failed to download audio: ${response.status}`);
    }
