...
app.get('/api/oembed', async (req, res) => {
    ...
    try {
        const result = await pool.query(`SELECT s.id, s.title, s.style, s.cover_url, s.duration,
              COALESCE(u.username, 'Anonymous') as creator
       FROM songs s
       LEFT JOIN users u ON s.user_id = u.id
       WHERE s.id = ? AND s.is_public = 1`, [match[1]]);
...
app.get('/song/:id', async (req, res) => {
    ...
    try {
        const result = await pool.query(`SELECT s.id, s.title, s.style, s.cover_url, s.audio_url, s.duration, s.like_count, s.view_count,
              COALESCE(u.username, 'Anonymous') as creator
       FROM songs s
       LEFT JOIN users u ON s.user_id = u.id
       WHERE s.id = ? AND s.is_public = 1`, [songId]);
        if (result.rows.length === 0) {
...
app.get('/api/pexels/photos', async (req, res) => {
    ...
    const apiKey = req.headers['x-pexels-api-key'] || config.pexels.apiKey;
    if (!apiKey) {
        res.status(400).json({ error: 'Pexels API key not configured. Please set your API key in the Video Generator settings.' });
        return;
    }
    try {
        const response = await fetch(`https://api.pexels.com/v1/search?query=${encodeURIComponent(query)}&per_page=20&orientation=landscape`, { headers: { Authorization: apiKey } });
        if (!response.ok) {
...
app.get('/api/pexels/videos', async (req, res) => {
    ...
    try {
        const response = await fetch(`https://api.pexels.com/videos/search?query=${encodeURIComponent(query)}&per_page=15&orientation=landscape`, { headers: { Authorization: apiKey } });
        if (!response.ok) {
            if (response.status === 401) {
                res.status(401).json({ error: 'Invalid Pexels API key' });
                return;
...
app.get('/api/search', async (req, res) => {
    ...
    try {
        ...
        if (!type || type === 'all' || type === 'songs') {
         ...
         LIMIT 20`, [searchPattern, searchPattern]);
            results.songs = songsResult.rows;
        }
        if (!type || type === 'all' || type === 'creators') {
            const creatorsResult = await pool.query(`SELECT u.id, u.username, u.bio, u.avatar_url, u.created_at,
