...
import { X, Link, Check, Image as ImageIcon } from 'lucide-react';
...
export const ShareModal: React.FC<ShareModalProps> = ({ isOpen, onClose, song }) => {
  ...
  const modalContent = (
    ...
    >
      {showCardView ? (
        /* Share Card View (for screenshotting) */
        <div className="bg-white dark:bg-zinc-900 rounded-3xl overflow-hidden max-w-sm w-full shadow-2xl border border-white/10 animate-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}>
          ...
          <div className="p-8 space-y-4">
            <div className="text-zinc-500 dark:text-zinc-400 text-sm leading-relaxed max-h-40 overflow-y-auto italic font-medium">
              {song.lyrics || "Music generated by ACE-Step UI"}
            </div>
            <div className="flex items-center justify-between pt-4 border-t border-zinc-200 dark:border-zinc-800">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] font-bold tracking-widest text-zinc-500 dark:text-zinc-400 uppercase">Japanese Ed.</span>
              </div>
              <button 
                onClick={() => setShowCardView(false)}
                className="px-4 py-2 rounded-full bg-zinc-100 dark:bg-white/10 text-zinc-600 dark:text-zinc-300 text-xs font-bold hover:bg-zinc-200 dark:hover:bg-white/20 transition-colors"
              >
                {t('back') || 'Back'}
              </button>
            </div>
        ...
        <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-white/10 rounded-xl w-full max-w-sm p-6 shadow-2xl animate-in fade-in zoom-in-95 duration-200">
          ...
          <div className="grid grid-cols-4 gap-3 mb-4">
            <button
              ...
              className="flex flex-col items-center gap-1.5 p-3 rounded-lg bg-gradient-to-br from-pink-500 to-purple-600 text-white hover:opacity-90 transition-opacity"
